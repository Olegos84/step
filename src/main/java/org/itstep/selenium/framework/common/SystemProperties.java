...
/**
 * Перечисление всех системных переменных, которые в обязательном порядке должны быть записаны в System.setProperty.
 * Сами имена(которые в верхнем регистре) это те имена, которые мы используем в коде. В скобках - имя под которым эта
 * переменная будет храниться в системе.
 *
